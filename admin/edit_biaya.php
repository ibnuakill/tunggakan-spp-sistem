...
// Ambil data lama
if ($id === '') {
    echo "<script>alert('ID biaya tidak ditemukan.'); window.location='data_biaya.php';</script>";
    exit();
}

$query_lama = "SELECT * FROM tabel_jenis_biaya WHERE id_biaya = '$id'";
$result_lama = mysqli_query($koneksi, $query_lama);
$data_lama = mysqli_fetch_assoc($result_lama);

if (!$data_lama) {
    echo "<script>alert('Data biaya tidak ditemukan.'); window.location='data_biaya.php';</script>";
    exit();
}
...
if (isset($_POST['update'])) {
    ...
    $query_update = "UPDATE tabel_jenis_biaya SET nama_biaya = '$nama_biaya', nominal = '$nominal', periode = '$periode' WHERE id_biaya = '$id'";
    if (mysqli_query($koneksi, $query_update)) {
        echo "<script>alert('Jenis biaya berhasil diubah!'); window.location='data_biaya.php';</script>";
        exit();
    } else {
...
<head>
    ...
    <title>Edit Jenis Biaya - Sistem Pembayaran SPP</title>
    ...
    <div id="content">
        <div class="container-fluid">
            <div class="d-flex justify-content-between align-items-center mb-4">
                <div>
                    <h2 class="mt-4 mb-2">
                        <i class="fas fa-tag text-warning"></i> Edit Jenis Biaya
                    </h2>
                    <p class="lead text-muted">Ubah data biaya: <strong><?php echo htmlspecialchars($data_lama['nama_biaya']); ?></strong></p>
                </div>
                ...
                <div class="col-12 col-md-8 col-lg-6">
                    <div class="card shadow-sm">
                        <div class="card-header bg-warning text-dark">
                            <h5 class="mb-0">
                                <i class="fas fa-edit"></i> Form Edit Jenis Biaya
                            </h5>
                        </div>
                        <div class="card-body">
                            <form action="" method="post" id="formEditBiaya">
                                <div class="mb-3">
                                    <label for="nama_biaya" class="form-label">
                                        <i class="fas fa-list text-primary"></i> Nama Biaya <span class="text-danger">*</span>
                                    </label>
                                    <input type="text" class="form-control" id="nama_biaya" name="nama_biaya" required value="<?php echo htmlspecialchars($data_lama['nama_biaya']); ?>">
                                </div>
                                ...
                                <div class="mb-3">
                                    ...
                                    <input type="text" class="form-control" id="nominal" name="nominal" required value="<?php echo htmlspecialchars($data_lama['nominal']); ?>">
                                ...
                                <div class="mb-3">
                                    ...
                                    <input type="text" class="form-control" id="periode" name="periode" required maxlength="20" value="<?php echo htmlspecialchars($data_lama['periode']); ?>">
                                </div>
